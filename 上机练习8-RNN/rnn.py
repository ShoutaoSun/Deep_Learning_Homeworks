...
class word_embedding(nn.Module):
    def __init__(self, vocab_length, embedding_dim):
        ...
        w_embedding_random_initial = np.random.uniform(-1, 1, size=(vocab_length, embedding_dim))  # 生成服从均匀分布的随机数
...
class RNN_model(nn.Module):
    def __init__(self, batch_sz, vocab_len, word_embedding, embedding_dim, lstm_hidden_dim):
        ...
        '''
            补全代码
        '''
        # here you need to define the "self.rnn_lstm"  the input size is "embedding_dim"
        # and the output size is "lstm_hidden_dim"
