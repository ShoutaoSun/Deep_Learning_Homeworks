...
def process_poems1(file_name):
    ...
    with open(file_name, "r", encoding='utf-8', ) as f:
        for line in f.readlines():
            try:
                title, content = line.strip().split(':')  # 题目与内容分开
                # content = content.replace(' ', '').replace('，','').replace('。','')
                content = content.replace(' ', '')  # 除杂
                if '_' in content or '(' in content or '（' in content or '《' in content or '[' in content or \
                                start_token in content or end_token in content:
                    continue
                if len(content) < 5 or len(content) > 80:
                    continue
                content = start_token + content + end_token  # 拼接诗句
...
def run_training():
    ...
    for epoch in range(30):
        ...
        for batch in range(n_chunk):
            ...
            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(rnn_model.parameters(), 1)  # 用于缓解梯度爆炸问题
            optimizer.step()
...
def gen_poem(begin_word):
    ...
    # use the other dataset to train the network
    ...
    while word != end_token:
        inputs = np.array([word_int_map[w] for w in poem], dtype=np.int64)
        inputs = Variable(torch.from_numpy(inputs))
        output = rnn_model(inputs, is_test=True)
